...
通常の開発においては、（１）ブランチを切ってからそのブランチで執筆し、（２）masterブランチにPull Requestをあげ、（３）レビューした結果マージするという形で開発を進めていくと聞いています。これに対し、このプロジェクトおよび親方Projectの他の本では、masterブランチ１本での運用と、セルフでのマージmasterブランチへの直Pushを推奨しています。

その理由は、次の三つです。

 * アプリなどと違って見えづらい・特定条件でのみ発動したりするような、レビューが必要なバグが混入することは滅多にない
 * master1本でやることで自然に他の人の目に触れるので、他の著者の目にもなんとなく触れることでレビューされやすくなる。
...
組版はReVIEWでやるのですが、使ったことのない人も当然います。そういう人には、プレーンテキストでも、WordでもMarkdownでも、なんでもいいから一番使い慣れたツールで出してください、というお願いをしています。編集部でコンバートすることも時々あります。
...
組版の環境構築は案外面倒です。Re:VIEWを使うにはRubyとTexをいれなければならなかったり、あるいはDockerを使いましょうという説明がされることがありますが、どんな環境、誰の環境でも絶対に動くという保証はありません。我々がやりたいのは、原稿を書いてPDFを見ることであり、環境構築ではありません。（そこで、環境構築に喜びを見出してしまう人が少なくないのは事実ですが）
...
実際、今でも、私おやかたは、ローカル環境でのRe:VIEWは使っていません。（一応作ってはあるが、うまく動かないことは多い）
